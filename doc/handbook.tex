...
The output of tables to prepare a text-based communication with another tool can work in complete analogy to the input. One defines a table and just by printing it to a file this will be directly readable by, say, R, Gnumeric or some other spreadsheet.

If the output format is not tab-delimited or in any way repetitive, e.g., a dendrogram, then core Lisp may be the perfect handler of that format, but not the routines that are currently provided by \CLCB.
